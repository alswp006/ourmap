## Task: Photos upload API (multipart) + EXIF/asset metadata extractor
Add multipart photo uploader for POST /api/photos and a small metadata extraction helper that normalizes takenAt/lat/lng from ImagePicker asset EXIF or asset location with explicit fallbacks. Enforce empty-string fields in FormData for missing metadata.
...
src/lib/api/photos.ts, src/lib/photos/extractMetadata.ts
