## Task: Screen: Trip Detail (view + update coverPhotoUrl + delete)
Implement Trip Detail screen loading /api/trips/:id with view state and a simple inline edit for name/date/description and optional coverPhotoUrl. Add Delete Trip with confirm. Handle 404 with back CTA and 401 redirect to login.
...
Create `src/__tests__/packet-0019.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0019.test.ts && git commit -m "test: TDD red phase — packet 0019"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0019.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0019"`
