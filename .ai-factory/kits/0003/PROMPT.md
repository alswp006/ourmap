## Task: Places Zustand store (in-memory list + details cache)
Create a focused Places store to cache paginated list items and place details in memory. Provide actions to set/append pages, upsert a detail response, and remove an item after deletion so screens can update instantly without refetching.
...
### Mobile Design Reference: E-commerce (inspired by Shopify, Amazon mobile)
**Visual Identity**
- Product-focused with high-quality images
- Trust signals: Badge for ratings, reviews count
- Sticky bottom bar for add-to-cart / checkout CTAs
- Clear price hierarchy with sale styling

**Key Patterns**
- Product cards: Image (aspect-square) → title → price → Badge rating
- Product detail: image carousel (horizontal FlatList + dots) → info → sticky bottom CTA
- Cart: FlatList with quantity steppers → order summary → checkout Button
- Category filters: horizontal ScrollView pills (Badge components)
- Search: search bar + recent + trending products

**Screen Examples**
- Home: search bar → category horizontal scroll → featured products grid (2-col)
- Product: image carousel → details ScrollView → sticky "Add to Cart" bar
- Cart: item FlatList → price summary Card → "Checkout" Button
