## Task: Entity + contract + UI-only types (Places/Trips/Photos/Geocode)
Add SPEC-aligned entity models and all request/response contracts for Places/Trips/Photos/Reverse Geocode plus local UI state types for photo selection and EXIF metadata extraction results. All files must remain type-only to prevent runtime side effects.
...
src/lib/types/entities.ts, src/lib/types/contracts.ts, src/lib/types/ui.ts
