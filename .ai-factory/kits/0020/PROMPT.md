## Task: Final wiring + polish (routes, tab labels, cross-links)
Wire navigation affordances across the app: ensure tabs point to Places/Map/Trips, add CTA links (Place List → Create, Create success → Detail, Detail back → List), and ensure 401 redirects are consistent. Minor UI polish: consistent padding, titles, and empty states.
...
## Design Quality
- Read .impeccable.md for project design context before starting UI work
- Consult .claude/skills/frontend-design/SKILL.md (note: mobile adaptation preamble at top)
- After implementing UI, verify against the anti-patterns list (AI Slop Test)
...
- Packet 0013: "Screen: Place List tab (search, pagination, swipe-to-delete, dev timing)" (files: app/(tabs)/places.tsx)
- Packet 0014: "Screen: Place Map tab (pins + summary + refresh + 401 handling)" (files: app/(tabs)/map.tsx)
- Packet 0016: "Screen: Place Create (photo pick, 10MB validation, upload, geocode, dev timer)" (files: app/places/new.tsx)
- Packet 0017: "Screen: Place Detail (view + inline edit toggle + delete)" (files: app/places/[id].tsx)
- Packet 0018: "Screen: Trip List tab (pagination + create/edit modal form)" (files: app/(tabs)/trips.tsx)
- Packet 0019: "Screen: Trip Detail (view + update coverPhotoUrl + delete)" (files: app/trips/[id].tsx)
...
Create `src/__tests__/packet-0020.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0020.test.ts && git commit -m "test: TDD red phase — packet 0020"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0020.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0020"`
