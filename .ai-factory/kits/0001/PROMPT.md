## Task: API primitive types (errors + pagination)
Define shared API primitive types used across all endpoint wrappers: standardized error payload and the generic pagination response. Keep the file type-only to avoid runtime bundling and ensure strict TS compatibility across the app.
