## Task: Screen: Place List tab (search, pagination, swipe-to-delete, dev timing)
Implement the Places List tab with initial skeletons, local search (no network), pull-to-refresh, infinite scroll pagination, 401 redirect to /login, and swipe-to-delete with confirm + API call. Include __DEV__ overlay showing 'Search compute' timing.
...
- Packet 0003: "Places Zustand store (in-memory list + details cache)" (files: src/store/placesStore.ts)
- Packet 0005: "Repository helpers (pure): sort + merge pagination" (files: src/lib/repos/listHelpers.ts)
