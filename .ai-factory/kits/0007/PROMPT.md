## Task: API error utilities (network vs server) + status extractor
Implement centralized API error helpers so screens can reliably render network error states and branch on HTTP status (401/404/429) without ad-hoc parsing. Include a status extractor that checks both e.status and e.response.status.
...
### Mobile Design Reference: Portfolio (inspired by Dribbble, Behance mobile)
**Visual Identity**
- Bold, visual-first — large images and strong typography
- Full-bleed images with overlay text
- Dark theme for dramatic presentation
- Smooth transitions between screens

**Key Patterns**
- Project cards: full-width Image (aspect-[4/3]) with overlay gradient + title
- Project detail: hero image → description → tech stack Badges → image gallery
- About: large Avatar → bio text → skills (Badge grid) → contact links
- Masonry-style grid using 2-column FlatList with varying heights
- Contact: minimal — icon buttons for social links

**Screen Examples**
- Home: animated greeting → featured project (full-width) → projects FlatList
- Project: hero image → case study ScrollView → related projects
- About: Avatar section → bio Card → skills → contact Card
...
- Packet 0001: "API primitive types (errors + pagination)" (files: src/lib/types/api.ts)
