...
App.tsx에 모든 페이지() Route 연결, main.tsx 수정 금지(@AI:ANCHOR), 광고/결제 연동, 최종 UX 폴리시
...
- Packet 0020: "Final wiring + polish (routes, tab labels, cross-links)" (files: app/(tabs)/_layout.tsx, app/(tabs)/index.tsx)
...
Create `src/__tests__/packet-0021.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0021.test.ts && git commit -m "test: TDD red phase — packet 0021"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0021.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0021"`
