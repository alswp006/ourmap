## Task: Screen: Place Create (photo pick, 10MB validation, upload, geocode, dev timer)
Implement Place Create form with defaults to minimize input (visitedAt=now, rating=5, tripId=null). Add photo picking, EXIF/asset metadata extraction, 10MB pre-upload validation, upload to /photos, and reverse geocode for a pin location with 429 handling. Show __DEV__ 'Create elapsed'.
...
- Packet 0006: "PRD QA docs + privacy guardrails doc + dev stopwatch helper" (files: docs/prd-goals-qa.md, docs/privacy-guardrails.md, src/lib/perf/stopwatch.ts)
- Packet 0007: "API error utilities (network vs server) + status extractor" (files: src/lib/api/errors.ts)
- Packet 0008: "Places API module (list/detail/create/update/delete)" (files: src/lib/api/places.ts)
- Packet 0010: "Photos upload API (multipart) + EXIF/asset metadata extractor" (files: src/lib/api/photos.ts, src/lib/photos/extractMetadata.ts)
- Packet 0011: "Reverse geocode API module (429/500 surfaced to UI)" (files: src/lib/api/geocode.ts)
- Packet 0012: "Shared UI components: Banner + Skeleton + RemoteImage wrapper" (files: src/components/ui/Banner.tsx, src/components/ui/Skeleton.tsx, src/components/ui/RemoteImage.tsx)
