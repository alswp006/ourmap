## Task: Reverse geocode API module (429/500 surfaced to UI)
Implement reverse geocoding API wrapper used by Place Create pin correction flow. Keep errors un-swallowed so screens can show specific messaging for 429 quota errors and allow manual entry fallback when geocode fails.
