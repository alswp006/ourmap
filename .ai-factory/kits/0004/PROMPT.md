## Task: Trips Zustand store (in-memory cache)
Create a Trips store to cache paginated trips list and trip objects by id. Include actions to set/append pages, upsert a trip from create/update, and remove a trip locally after deletion so the UI can reflect changes immediately.
