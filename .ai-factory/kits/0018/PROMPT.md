## Task: Screen: Trip List tab (pagination + create/edit modal form)
Implement Trips List tab with paginated fetch, pull-to-refresh, and empty/error states. Include an in-screen Modal containing the Trip create/edit form (name, start/end dates, description). After create/update, update store and close modal without full refetch.
...
- Packet 0004: "Trips Zustand store (in-memory cache)" (files: src/store/tripsStore.ts)
- Packet 0007: "API error utilities (network vs server) + status extractor" (files: src/lib/api/errors.ts)
- Packet 0009: "Trips API module (list/create/get/update/delete)" (files: src/lib/api/trips.ts)
...
Create `src/__tests__/packet-0018.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0018.test.ts && git commit -m "test: TDD red phase — packet 0018"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0018.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0018"`
