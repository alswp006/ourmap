## Task: PRD QA docs + privacy guardrails doc + dev stopwatch helper
Add lightweight manual QA docs for PRD time goals and privacy boundaries, plus a dev-only stopwatch helper used by screens for timing overlays. Stopwatch must be side-effect free and safe for production builds (used only under __DEV__).
...
docs/prd-goals-qa.md, docs/privacy-guardrails.md, src/lib/perf/stopwatch.ts
